...
def route_path(data, handler):
    ...
    if path == "/":
        ...
        return get_html_file("/src/html/settings.html", headers)
    elif path == "/static/scripts/settings.js":
        return get_js(path)
    elif path == "/static/styles/settings.css":
        return get_css(path)
    elif path == "/users":
        if type == "GET":
            return retrieve_all()
